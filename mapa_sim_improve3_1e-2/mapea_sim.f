	...
	double precision eps, y1(16), ysimu(nmaxeqs,16), yprob(nmaxeqs,2)
	...
	do i=1,16
	  write(90,'(2I8,16F8.4)') i, nodobooinv(i), (X(i,j),j=1,16)
	enddo
...
c     +.and.((j.eq.5).or.(j.eq.5))) then
		...
		if (X(i,k).ne.0) y1(k)=X(i,k)-(eps/nX(i))
	      ...
	      call simuevol(node,y1,ysimu,yprob,ndist)
		    !do l=1,ndist
			!print*,l
			!print*,(ysimu(l,k),k=1,16)
			!print*,'---', (yprob(l,k),k=1,2)
		    !enddo
 ...
 50	continue
	...
	do i=17,nnodos
	  ...
	  do k=1,16
            node=node+(ceiling(X(i,k)))*(2**(k-1))
	  enddo
	  write(90,'(2I8,16F8.4)') i,node,(X(i,k),k=1,16)
	enddo
	...
	integer i,j,ndist
	integer, parameter :: nmaxeqs=999
	double precision ysimu(nmaxeqs,16), yprob(nmaxeqs,2)

	double precision mediax(16), critsemnode
	...
	do id=1,ndist
     ...
     +							then
		  ...
		  if(lk(inode).eq.0) then  ! llega al final de lk parcial
		    ...
		    print*,(X(nnodos,k),k=1,16)
		    print*,(X(inode,k),k=1,16)
		    exit
		  else
		    inode=lk(inode)
		  endif
	...
	implicit none
	double precision yini(16), ysimu(999,16),yprob(999,2)
	integer ndist,nodeini

	integer nsim,isim,nindiv,seed,np,k,k2,yp(16),ntp,ip,jp,ic,ip2,
     +			p1,p2,npi,ibus,ypi(16),ypant(16), yt
	integer iit, yindex(16),yindexi(16), difer  ,nmaxit,
     +		 noderes, cibus   !!!, startconv, nitsum,itsum,	
	real*8 rand
	double precision A(16,16),Ai(16,16), numran, difpaymax, calcdpm,
     +   		     yac, difpaymaxi, pp,
     +		 	 Pri,Prij, payi, payj, paymed,denoPri,numPri,Pac,
     +				probfin, probcamb, sumyp(16), sumypant(16),
     +				 critsemix, critsemyp, mediayp(16), probcut, 
